...
export default function ChallengeTab() {
  ...
  const handleShare = (text: string, platform: string) => {
    ...
    if (platform === 'copy') {
      try {
        navigator.clipboard.writeText(full).then(() => alert('已複製！'))
      } catch {
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, marginBottom: 16 }}>
            {([7, 30, 60, 100] as ChallengeMode[]).map(m => (
              <button key={m} onClick={() => setPendingMode(m)} style={{ padding: '24px 12px', border: `1.5px solid ${pendingMode === m ? sg : bd}`, borderRadius: 12, background: pendingMode === m ? '#EAF2EE' : ww, cursor: 'pointer', textAlign: 'center', transition: 'all 0.15s' }}>
          ...
          {pendingMode && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ background: '#FDF9F0', border: '1px solid #C4953A', borderRadius: 10, padding: '12px 16px', marginBottom: 12, fontSize: 13, color: '#7A5E2A', lineHeight: 1.7 }}>
                ⚠️ 開始後無法中斷——若中途停止，需從第 1 天重新開始。請確認你已準備好每天打卡！
              </div>
              <button
          ...
          <div style={{ background: ww, border: `1px solid ${bd}`, borderRadius: 12, padding: '20px 24px' }}>
            ...
            {[
              ...
              ['中間不能斷', '挑戰期間若中斷，需從第1天重新開始。期滿後挑戰自動結束，可立即開始新的一輪'],
        ...
        <>
          ...
          {challengeDone && (
            <div style={{ background: '#EAF2EE', border: `1px solid ${sg}`, borderRadius: 12, padding: '20px 24px', marginBottom: 16, textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: "'Noto Serif TC', serif", fontSize: 18, color: sg, marginBottom: 6 }}>挑戰完成！</div>
              <div style={{ fontSize: 13, color: ml, marginBottom: 16 }}>恭喜完成 {mode} 天挑戰，你太厲害了！</div>
              <button onClick={handleResetChallenge} style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: ink, color: 'white', fontSize: 14, cursor: 'pointer', fontWeight: 600 }}>
                開始新的一輪
              </button>
            </div>
          )}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {Array.from({ length: mode }).map((_, i) => (
                <div key={i} style={{ width: 24, height: 24, borderRadius: 6, background: i < entries.length ? sg : i === entries.length ? '#F0E8D0' : cr, border: i === entries.length ? '1.5px dashed #C4953A' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: i < entries.length ? 'pointer' : 'default' }}
                  ...
                  {i === entries.length && !challengeDone && <span style={{ fontSize: 8, color: '#C4953A' }}>今</span>}
                </div>
              ))}
            </div>
          </div>
